...
contextBridge.exposeInMainWorld("centauri", {
  ...
  chooseOverlayImage: () => ipcRenderer.invoke("choose-overlay-image"),
  ...
  saveSecrets: (destinations) =>
    ipcRenderer.invoke("save-secrets", destinations),
  ...
  onDestinationState: (cb) =>
    ipcRenderer.on("destination-state", (_, value) => cb(value)),
  onReconnecting: (cb) =>
    ipcRenderer.on("stream-reconnecting", (_, value) => cb(value)),
  onReconnected: (cb) =>
    ipcRenderer.on("stream-reconnected", (_, value) => cb(value)),
  onRecordingStarted: (cb) =>
    ipcRenderer.on("recording-started", (_, value) => cb(value)),
  onStats: (cb) => ipcRenderer.on("stream-stats", (_, value) => cb(value)),
  onInstallLog: (cb) =>
    ipcRenderer.on("ffmpeg-install-log", (_, value) => cb(value)),
  onInstallEnded: (cb) =>
    ipcRenderer.on("ffmpeg-install-ended", (_, value) => cb(value)),
});
